...
module.exports = (robot: Robots) => {
  robot.hear(memoRegexp.add, (res) => {
    ...
    if (user.bot) return
    res.send(`@BOT_kashiwade memo add ${plainText.replace(memoRegexp.add, '')}`)
  })
  ...
  robot.hear(memoRegexp.edit, (res) => {
    ...
    res.send(`@BOT_kashiwade memo edit ${num} ${txt}`)
  })
  ...
  robot.hear(memoRegexp.finish, (res) => {
    ...
    if (user.bot) return
    res.send(
      `@BOT_kashiwade memo finish ${plainText.replace(memoRegexp.finish, '')}`
    )
  })
